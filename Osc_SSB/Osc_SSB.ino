// Simple single sideband LUT oscillator //
...
void sample() {
  ...
  float carrierI = cosTable[idxC];
  float carrierQ = sineTable[idxC];

  phaseM += phaseIncM;
  if (phaseM >= TABLE_SIZE) phaseM -= TABLE_SIZE;
  int idxM = (int)phaseM & (TABLE_SIZE - 1);
  float modI = cosTable[idxM];
  float modQ = sineTable[idxM];

  float val = carrierI * modI - carrierQ * modQ;
