// Sine wave Markov chain shaper //

#include "CurieTimerOne.h"

#define AUDIO       5
#define SAMPLE_RATE 44100
#define BPM         120
#define TABLE_SIZE  1024
#define NUM_STATES  5

int time;
const int oneSecInUsec = 1000000;

float sineTable[TABLE_SIZE];

float phase = 0.0f;
float phaseInc = 0.0f;
float freq = 440.0f;

float states[NUM_STATES] = {-1.0f, -0.5f, 0.0f, 0.5f, 1.0f};
int currentState = 2;

float transition[NUM_STATES][NUM_STATES] = {

  {0.6f, 0.2f, 0.1f, 0.1f, 0.0f},
  {0.2f, 0.5f, 0.2f, 0.1f, 0.0f},
  {0.1f, 0.2f, 0.4f, 0.2f, 0.1f},
  {0.0f, 0.1f, 0.2f, 0.5f, 0.2f},
  {0.0f, 0.1f, 0.1f, 0.2f, 0.6f}

};

static inline float markovShape(float bias) {

  float trigger = (bias + 1.0f) * 0.5f;
  float r = (float)random(0, 1000) / 1000.0f;

  r *= trigger;  

  float sum = 0.0f;
  for (int j = 0; j < NUM_STATES; j++) {
    sum += transition[currentState][j];
    if (r < sum) {
      currentState = j;
      break;
    }
  }

  return states[currentState];

}


void sample() {

  int idx = (int)phase & (TABLE_SIZE - 1);
  float val = sineTable[idx];
  float shaped = markovShape(val);

  uint16_t out = (uint16_t)((1.0f + shaped) * 32767.0f);
  analogWrite(AUDIO, out);

  phase += phaseInc;
  if (phase >= TABLE_SIZE) phase -= TABLE_SIZE;

  CurieTimerOne.restart(time);

}

void setup() {

  pinMode(AUDIO, OUTPUT);

  analogWriteResolution(16);
  analogWriteFrequency(AUDIO, SAMPLE_RATE);

  for (int i = 0; i < TABLE_SIZE; i++) sineTable[i] = sinf(2.0f * PI * i / TABLE_SIZE);

  time = oneSecInUsec / SAMPLE_RATE;
  CurieTimerOne.start(time, &sample);

}

void loop() {

  freq = random(100, 1000);
  phaseInc = (freq * TABLE_SIZE) / SAMPLE_RATE;

  int tempo = 60000 / BPM;
  delay(tempo / 4);

}