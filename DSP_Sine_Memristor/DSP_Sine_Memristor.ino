...
static inline float memristorShaper(float v, float memCoeff) {

  memState += memCoeff * v;
  memState = fmaxf(fminf(memState, 1.0f), -1.0f);

  float g = 1.0f / (1.0f + expf(-5.0f * memState));
  float out = g * v;
...
void sample() {
  ...
  float shaped = memristorShaper(val, 0.5f);
